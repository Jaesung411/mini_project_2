...
def search():
    ...
    if query:
        ...
        # 검색어가 없으면 빈 결과 반환 (아무것도 안 뜨게)
        filtered_list = []

    # 필터링 결과가 없다면 빈 리스트 그대로 반환
    # 페이지네이션 처리
    page = request.args.get('page', 1, type=int)
    per_page = 6
    total_pages = (len(filtered_list) - 1) // per_page + 1 if filtered_list else 1

    # 필터링 결과가 없거나 페이지네이션 범위를 벗어나면 빈 리스트
    paginated_list = filtered_list[(page - 1) * per_page : page * per_page] if filtered_list else []
    pages = get_pagination(page, total_pages) if filtered_list else []
